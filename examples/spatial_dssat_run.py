...
def sample_dem_elevations(dem_path: str | None, lats: list, lons: list) -> dict:
    if dem_path is None or not os.path.exists(dem_path):
        return {(lat, lon): 0.0 for lat, lon in zip(lats, lons)}
    elev_map: dict = {}
    with rasterio.open(dem_path) as src:
        ...
        for lat, lon in zip(lats, lons):
            try:
                ...
                val = band[row, col]
                elev_map[(lat, lon)] = 0.0 if (nodata is not None and val == nodata) else float(val)
            except Exception:
                elev_map[(lat, lon)] = 0.0
    return elev_map
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DSSAT real-data simulation")
    parser.add_argument("--config", default="options/test_dssat.yaml", help="Path to YAML config file")
    ...
    elev_map = sample_dem_elevations(cfg.SPATIAL_INFO.dem_path, ys, xs)
    ...
    print(f"DSSAT_HOME: {cfg.GENERAL_INFO.dssat_path}/DSSAT048/\n")
    ...
    if os.path.exists(cfg.SPATIAL_INFO.output_path):
        os.remove(cfg.SPATIAL_INFO.output_path)
    ds_out.to_netcdf(cfg.SPATIAL_INFO.output_path)
    print(f"Saved -> {cfg.SPATIAL_INFO.output_path}")
    print(ds_out)
