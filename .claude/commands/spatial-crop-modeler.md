...
Running a full country is expensive. If the user specifies a district, department, or region (e.g. "only Zomba district", "just the Comayagua department"), use the `feature` parameter:

- `list_admin_units(country_code="MWI", adm_level=2)` → shows available district names
- `generate_config(..., feature="Zomba", adm_level=2)` → config carries the boundary
- `run_simulation(config_path=..., feature="Zomba", adm_level=2)` → clips weather and soil to that polygon before building the pixel list

The boundary is downloaded automatically from GeoBoundaries (no local shapefile needed). This can reduce pixel count from thousands to tens, cutting run time by 10–100×.
...
Parallelise at year level (one thread per year, sequential days within year).
Day-level parallelism triggers CrowdSec HTTP 403 on `data.chc.ucsb.edu`.
